...
const AdminChangePassword = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setError(err?.data?.message || "Unable to update password.");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-paper text-ink flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-lg rounded-[32px] border border-black/10 bg-white p-10 shadow-xl">
        <div className="mb-8">
          <p className="text-xs uppercase tracking-[0.35em] text-sand">Admin security</p>
          <h1 className="mt-4 text-3xl font-semibold">Change admin password</h1>
          <p className="mt-2 text-sm text-gray-500">Protect your admin account with a stronger password.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex items-center gap-2 text-sm text-slate-600">
            ...
            />
            <label htmlFor="show-admin-password">Show passwords</label>
          </div>
