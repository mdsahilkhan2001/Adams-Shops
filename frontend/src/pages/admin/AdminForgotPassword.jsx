...
const AdminForgotPassword = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setError(err?.data?.message || "Unable to send recovery email.");
    }
  };

  return (
    <div className="min-h-screen bg-paper text-ink flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md rounded-[32px] border border-black/10 bg-white p-10 shadow-xl">
        <div className="mb-8">
          <p className="text-xs uppercase tracking-[0.35em] text-sand">Admin password reset</p>
          <h1 className="mt-4 text-3xl font-semibold">Forgot your admin password?</h1>
          <p className="mt-2 text-sm text-gray-500">Enter the email for your admin account and follow the reset instructions.</p>
        </div>
        ...
        <p className="mt-6 text-center text-sm text-slate-500">
          Return to <Link className="text-red-600 hover:text-red-700" to="/admin/login">Admin login</Link>
        </p>
      </div>
    </div>
  );
